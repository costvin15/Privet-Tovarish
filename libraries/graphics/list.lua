...
function List:add(title, description, code, action, id)
    local item = {}
    item.title = title
    item.description = description
	item.code = code
    item.action = action or function() end
...
function List:getIds()
	...
	for i = 1, #self.items do
		table.insert(t, self.items[i].code)
	end
	return t
end
...
function List:mousepressed(x, y, b, it)
	...
	--if type(self.onclick) ~= "function" then print(type(self.onclick), self.onclick) return end
